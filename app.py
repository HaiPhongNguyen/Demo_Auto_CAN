...
class CANBusApp(QMainWindow):
    ...
    def initUI(self):
        ...
        text_block_layout.setSpacing(2)
        ...
        main_layout.addWidget(
            QLabel("<hr style='border: 1px solid #bbb;'>")
        )
        ...
        self.progress_soc.setRange(0, 100)
        ...
        self.speed_slider.valueChanged.connect(
            lambda v: self.lbl_speed_cmd.setText(
                f"Tốc độ cài đặt: {v}"
            )
        )
        ...
        self.btn_fwd.clicked.connect(
            lambda: self.send_command("FWD")
        )

        self.btn_rev.clicked.connect(
            lambda: self.send_command("REV")
        )
        ...
        # ================= LOG =================
        main_layout.addWidget(
            QLabel("<b>LOG DỮ LIỆU CAN BUS:</b>")
        )

        self.log_screen = QTextEdit()
        self.log_screen.setReadOnly(True)

        self.log_screen.setStyleSheet("""
    ...
    def toggle_connection(self):
        if self.serial_port is None or not self.serial_port.is_open:
            try:
                ...
                self.write_log(
                    "SYSTEM",
                    f"Đã kết nối {self.port_combo.currentText()}"
                )
    ...
    def write_log(self, direction, message):
        ...
        color = (
            "#00ff00" if direction == "RX"
            else "#ffaa00" if direction == "TX"
            else "#ffffff"
        )

        self.log_screen.append(
            f"<span style='color:gray'>[{t}]</span> "
            f"<span style='color:{color}'>"
            f"<b>{direction}:</b> {message}</span>"
        )

        self.log_screen.moveCursor(
            self.log_screen.textCursor().End
        )
    ...
    def send_command(self, direction):
        ...
        self.write_log(
            "TX",
            f"ID=0x105 DIR={dir_byte} SPEED={speed}"
        )
    ...
    def parse_can_data(self, data):
        try:
            ...
            # ===== DOOR =====
                ...
                self.lbl_door.setText(
                    f"CỬA: {'ĐANG MỞ' if is_open else 'ĐANG ĐÓNG'}"
                )

                self.lbl_door.setStyleSheet(
                    f"""
                    background-color:
                    {'#D32F2F' if is_open else '#2E7D32'};
                    color: white;
                    font-weight: bold;
                    padding: 15px;
                    border-radius: 5px;
            ...
            # ===== BUZZER =====
                ...
                self.lbl_buzzer.setText(
                    f"BUZZER: {'ON' if is_on else 'OFF'}"
                )

                self.lbl_buzzer.setStyleSheet(
                    f"""
                    background-color:
                    {'#D32F2F' if is_on else '#eee'};

                    color:
                    {'white' if is_on else 'black'};
